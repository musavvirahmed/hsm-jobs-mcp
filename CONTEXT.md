...
- **Honesty fields** — independent structured signals about JD silence on an **Opening**: **salary signal**, **Dutch-required**, **sponsorship willingness**. In tool JSON they are flat sibling keys: `honesty_salary`, `honesty_dutch_required`, `honesty_sponsorship_willingness` (not nested under an `honesty` object). Never collapsed into one “HSM fit” score. Return-only in v1 (not search filters). No evidence quotes in the payload (enums / raw salary text only). No role-plausibility field; search ranking is text relevance to the query (a bad title loses without an extra “poor HSM” flag). **Register join** and **source class** stay top-level beside them (identity / provenance, not JD silence). _Avoid_: uncertainty badge, badge (as a domain noun), confidence pretending to be a fact; server-side filter args on honesty enums; nesting honesty under a wrapper object.
