...
- **Honesty fields** — independent structured signals on an **Opening** (**register join**, **salary signal**, **Dutch-required**, **sponsorship willingness**). Never collapsed into one “HSM fit” score. Return-only in v1 (not search filters). No evidence quotes in the payload (enums / raw salary text only). _Avoid_: uncertainty badge, badge (as a domain noun), confidence pretending to be a fact; server-side filter args on honesty enums.
...
- **Dutch-required** — `true` | `false` | `unknown` on an **Opening**. Only whether Dutch is required; other languages are not first-class. Silent JD or missing body → `unknown`. Scope: this vacancy’s JD/ATS fields only. _Avoid_: inferring from HQ country or “international team.”
- **Sponsorship willingness** — whether *this* **Opening** states it will / will not help with sponsorship or an **HSM transfer**: `stated_yes` | `stated_no` | `unknown`. Distinct from **recognised sponsor** (employer licence). No `inferred_*`. Scope: this vacancy’s JD/ATS fields only (not careers FAQ / policy pages). Soft culture copy → `unknown`. Missing body → `unknown`. _Avoid_: treating licence as willingness; inventing yes from boilerplate.
